...
function ballMove() {
  //// paddle checking code moved outside of edge checks, and given a horizontal dimension
  ...
  if(ballX < 0) { // if ball has moved beyond the left edge //// no longer checks paddle
    paddle2Score++;
    ballMissSound.play();
    ballReset();
  }
  
  if(ballX > canvas.width) { // if ball has moved beyond the right edge //// no longer checks paddle
    paddle1Score++;
    ballMissSound.play();
    ballReset();
  }
